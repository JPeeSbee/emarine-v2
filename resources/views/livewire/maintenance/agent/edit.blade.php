    ...
    <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
        ...
        <form class="max-w-xs md:max-w-lg mx-auto my-12" wire:submit='update'>
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-5 group">
                    <input wire:model='code' type="text" name="code" id="code" value="{{ old('code', $agent->code) }}" class="block py-2.5 px-2 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input wire:model='name' type="text" name="name" id="name" value="{{ old('name', $agent->name) }}" class="block py-2.5 px-2 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input wire:model='email' type="email" name="email" id="email" value="{{ old('email', $agent->email) }}" class="block py-2.5 px-2 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select wire:model='location_id' name="location_id" id="underline_select" class="block w-full py-2.5 px-2 text-sm text-gray-900 border-0 border-b-2 border-gray-300 bg-transparent focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        @foreach ($locations as $location)
                            <option value="{{$location->id}}" class="text-black dark:text-white">{{$location->name}}</option>
                        @endforeach
                    </select>
                    @error('location_id')
                        <em class="text-red-300 text-xs">{{ $message }}</em>
