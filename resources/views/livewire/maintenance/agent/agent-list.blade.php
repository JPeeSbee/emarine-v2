            ...
            @else
                <div class="relative grid grid-cols-4 m-2">
                    ...
                    <div class="col-span-2 col-start-3 flex justify-end space-x-2">
                        <button type="button" class="focus:outline-none text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm dark:focus:ring-green-900" title="Create Agent" wire:click.prevent="create">
                    ...
                    <tbody>
                        @foreach ($agents as $key => $agent)
                            <tr wire:key="{{ $agent->id }}" class="@if($key % 2 == 0) bg-gray-200 dark:bg-gray-600 @else bg-white dark:bg-gray-800 @endif border-b dark:border-gray-700 border-gray-200">
                                ...
                                <td>
                                    <button type="button" class="focus:outline-none text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm dark:focus:ring-blue-900" title="View Details" wire:click.prevent="show({{ $agent->id }})">
                                        <flux:navlist.item icon="magnifying-glass" href="#" />
                                    </button>
                                </td>
                                <td>
                                    <button type="button" class="focus:outline-none text-white bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm dark:focus:ring-yellow-900" title="Edit Agent" wire:click.prevent="edit({{ $agent->id }})">
                                        <flux:navlist.item icon="pencil-square" href="#" />
                                    </button>
                                </td>
                                <td>
                                    <button type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" title="Delete Agent" wire:click.prevent="deleteAgent({{ $agent->id }})" wire:confirm="Are you sure you want to delete this agent?">
